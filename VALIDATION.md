...
- **Build gating**: `npm run docker:build` runs `docker:check` first; the preflight fails with an actionable error if the image does not exist.
...
- [ ] Docker image build, Chromium launch, and bind mount — CI (`ci.yml`) validates full flow on unrestricted runners; local sandbox has credential issues.
